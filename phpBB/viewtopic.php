...
$template->assign_vars(array(
	...
	'U_EMAIL_TOPIC'			=> ($auth->acl_get('f_email', $forum_id) && $config['email_enable']) ? $controller_helper->route('phpbb_message_topic', ['topic_id' => $topic_id]) : '',
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			*/
			...
			{
				$user_cache[$poster_id]['email'] = ($config['board_email_form'] && $config['email_enable']) ? $controller_helper->route('phpbb_message_user', ['user_id' => $poster_id]) : (($config['board_hide_emails'] && !$auth->acl_get('a_email')) ? '' : 'mailto:' . $row['user_email']);
			}
			else
			{
